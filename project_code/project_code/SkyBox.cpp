...
void SkyBox::render(Camera cam)
{
	...
	int t = 1;  // remove
	Matrix4f viewMat, projMat, modelMat, rotMat, modelViewProjMat;
	...
	rotMat = Matrix4f::rotateRollPitchYaw(this->rollAngle, this->pitchAngle, this->yawAngle, 1);
	modelMat = rotMat * modelMat;

	// get the transformation matrix 
	modelMat = Matrix4f::translation(cam.getPosition()) * modelMat;
...
void SkyBox::incrementRotations(float rollAngle, float pitchAngle, float yawAngle)
{
...
{
	int rc;
	rc = skyboxShader.createShader(vsFileName, fsFileName);
	if (rc != 0) {
